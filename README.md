...
- 線上版本：https://mind.milifun.net （工作室需要站台密碼，分享頁 `/m/*` 免登入；舊網址 mindmap.go38.workers.dev 已關閉）
...
- 左側三條線按鈕可循環切換心智圖、樹狀與大綱三種檢視。

## v0.28.2 重點

- 修正 Safari 點擊「更多工具」後，面板被左側捲動工具列裁切而無法看見的問題。

## v0.28.1 重點

- 左側工具列只保留新增、AI 自動產圖、靈感收件匣、復原／重做、大綱與更多工具等高頻入口。
- 分支管理、範本、匯入、知識工具與偏好設定集中在「更多工具」面板，功能不減少、主列更短。
- 工具列採上下分區與緊湊歷史控制，短視窗及手機可直接使用底部工具。
